...
function wptools_menu()
{
	...
	add_submenu_page(
		...
		'Check mySQL Tables', // string $menu_title
	...
	add_submenu_page(
		...
		'Edit robots txt', // string $menu_title
		'manage_options', // string $capability
	...
	add_submenu_page(
		...
		'Show PHP Errors', // string $menu_title
		'manage_options', // string $capability
	...
	add_submenu_page(
		...
		'Show PHP Info', // string $menu_title
		'manage_options', // string $capability
	...
	add_submenu_page(
		...
		'Show .htaccess', // string $menu_title
		'manage_options', // string $capability
	...
	add_submenu_page(
		...
		'Show wp-config.php', // string $menu_title
		'manage_options', // string $capability
	...
	add_submenu_page(
		...
		'Show Cron Jobs', // string $menu_title
		'manage_options', // string $capability
	...
	add_submenu_page(
		...
		'Show File Permissions', // string $menu_title
		'manage_options', // string $capability
	...
	add_submenu_page(
		...
		'Show Cookies', // string $menu_title
		'manage_options', // string $capability
	...
	add_submenu_page(
		...
		'Server Benchmark', // string $menu_title
		'manage_options', // string $capability
	...
	add_submenu_page(
		...
		'Database Detais', // string $menu_title
		'manage_options', // string $capability
	...
	add_submenu_page(
		...
		'Show Transients', // string $menu_title
		'manage_options', // string $capability
	...
	add_submenu_page(
		...
		'Javascrip and jQuery', // string $menu_title
		'manage_options', // string $capability
	...
	if (is_multisite()) {
		...
		add_submenu_page(
			...
			'More New Tools', // string $menu_title
			'manage_options', // string $capability
			...
			<tbody class="wptools-more-plugins-body">
				...
				for ($i = 0; $i < $total; $i++) {
					...
					if (wptools_plugin_installed($plugins_to_install[$i]["slug"]))
						...
						echo '<a href="#" id="' . esc_attr($plugins_to_install[$i]["slug"]) . '"class="button button-primary wt-bill-install-now">Install</a>';
					echo '</td>';
					if ($counter % 2 == 1) {
...
?>
	...
	<table class="wptools_admin_table">
		<tr>
			<td><?php echo esc_attr('jQuery Version', 'wptools'); ?></td>
			<td>
				<div id='jquery_version'></div>
			</td>
		</tr>
		<tr>
			<td><?php echo esc_attr('jQuery Migrate Version', 'wptools'); ?></td>
			<td>
	...
	echo '</a>';
	?>
	<h2><?php echo esc_attr('Last Javascript Errors (max 200)', 'wptools'); ?></h2>
	<?php
	$wptools_filename = ABSPATH . "error_log";
	$marray = wptools_read_file($wptools_filename, 3000);
	if (count($marray) < 1) {
		echo '<h3>';
		echo esc_attr('No Javascript errors found last 3000 entries of log_error file.', 'wptools');
		echo '</h3>';
		return;
	}
	echo '<br>';
...
function wptools_sql_details()
{
	...
	if (!defined('WP_ALLOW_REPAIR')) {
		...
		if (WP_ALLOW_REPAIR == false) {
			echo '<br>';
			echo 'WP_ALLOW_REPAIR is defined as false on your wp-config.php';
			echo '<br>';
			echo 'That can disable WordPress Database Optimize and Repair Tool.';
			echo '<br>';
			echo 'Change that to true.';
...
?>
	<h1><?php echo esc_attr('Database Information', 'wptools'); ?></h1>
	<hr />
	<h2><?php echo esc_attr('Basic Information', 'wptools'); ?></h2>
	<table class="wptools_admin_table">
		<thead>
			<tr>
				<th><?php echo esc_attr('Variable Name', 'wptools'); ?></th>
				<th><?php echo esc_attr('Value', 'wptools'); ?></th>
			</tr>
		</thead>
		<tfoot>
			<tr>
				<td><?php echo esc_attr('Variable Name', 'wptools'); ?></td>
				<td><?php echo esc_attr('Value', 'wptools'); ?></td>
			</tr>
		</tfoot>
		<tbody>
			<tr>
				<td><?php echo esc_attr('Database Software', 'wptools'); ?></td>
				<td><?php echo esc_attr(wptools_database_software()); ?></td>
			</tr>
			<tr>
				<td><?php echo esc_attr('Database Version', 'wptools'); ?></td>
				<td><?php echo esc_attr(wptools_database_max_no_connection()); ?></td>
			</tr>
			<tr>
				<td><?php echo esc_attr('MySQL Uptime', 'wptools'); ?></td>
				<td>
					<?php
			...
			<tr>
				<td><?php echo esc_attr('Mysql Time', 'wptools'); ?></td>
				<td>
					<?php
			...
			<tr>
				<td><?php echo esc_attr('Database Name', 'wptools'); ?></td>
				<td>
					<?php
			...
			<tr>
				<td><?php echo esc_attr('Maximum No. of Connections', 'wptools'); ?></td>
				<td><?php echo esc_attr(database_max_no_connection()); ?></td>
			</tr>
			<tr>
				<td><?php echo esc_attr('Maximum Packet Size', 'wptools'); ?></td>
				<td><?php echo esc_attr(wptools_database_max_packet_size()); ?></td>
			</tr>
			<tr>
				<td><?php echo esc_attr('Database Disk Usage', 'wptools'); ?></td>
				<td><?php echo esc_attr(wptools_database_disk_usage()); ?></td>
			</tr>
			<tr>
				<td><?php echo esc_attr('Index Disk Usage', 'wptools'); ?></td>
				<td><?php echo esc_attr(wptools_index_disk_usage()); ?></td>
			</tr>
		</tbody>
	</table>
	<br />
	<h2><?php echo esc_attr('Advanced Information', 'wptools'); ?></h2>
	<table class="wptools_admin_table">
		<thead>
			<tr>
				<th><?php echo esc_attr('Variable Name', 'wptools'); ?></th>
...
?>
	...
	<form method="post" class="alignleft" style="min-width:100%">&nbsp;
		...
		<label for="scales">
			Participate in Community Server Performance. <br>
			Enabling this feature causes your site to share only server performance data with WP Tools Plugin. <br>
			In return your WordPress site receives Updated Aggregated Industry Average Data.<br>
		</label>
	...
	echo esc_attr(__("Learn more.", 'wptools'));
	echo '</a>';
	...
	if ($wptools_server_performance == 'yes') {
		echo '<div style="float:left">';
		echo wptools_print_html_result($benchmarkIndustryResult, $showServerName, 'Industry Average Data');
		echo '</div>';
	}
}
...
function wptools_options_check_table()
{
	...
	<table class="wptools_admin_table">
		<tr>
			...
			<th><strong>Data Length (Aproximate)</strong></th>
		</tr>
		<?php
	...
	<hr>
	<h4>For more info about your database, look the page SHOW PHP INFO.
	<?php
}
...
function wptools_php_max_upload_size()
{
	if (ini_get('upload_max_filesize')) {
		...
		$php_max_upload_size = __('N/A', 'wptools');
	}
...
function wptools_php_max_post_size()
{
	if (ini_get('post_max_size')) {
		...
		$php_max_post_size = __('N/A', 'wptools');
	}
...
function wptools_options()
{
	...
	echo '<h1>' . __('Errors', 'wptools') . '</h1>';
	echo '<center>';
	echo '<h2>';
	echo __("The lasts 200 lines of the log files.", "wptools");
	echo '</h2>';
	echo '</center>';
	echo '<h4>';
	...
	foreach ($wptools_folders as $wptools_folder) {
		foreach (glob($wptools_folder) as $wptools_filename) {
			if (strpos($wptools_filename, 'backup') != true) {
				echo '<hr>';
				echo '<strong>';
				echo wptools_sizeFilter(filesize($wptools_filename));
				echo ' - ';
				echo esc_attr($wptools_filename);
				echo '</strong>';
				$wptools_count++;
				$marray = wptools_read_file($wptools_filename, 200);
				if (count($marray) > 0) {
...
function wptools_plugin_settings_link($links)
{
	$settings_link = '<a href="admin.php?page=wp-tools">Settings</a>';
...
function wptools_options_wp_config()
{
	...
	echo 'File path: ' . esc_attr($file);
	echo '<br>';
	$txt = '';
	if (!file_exists($file)) {
		echo 'File wp-config.php not found. Ask to your hosting company if is hidden.';
		echo '<br>';
	} else {
		$txt = trim(file_get_contents($file, true));
		if (empty($txt)) {
			echo 'Unable to read the file wp-config.php! Ask to your hosting company.<br>';
		}
	}
	echo '<form>';
	?>
...
function wptools_options_permissions()
{
	...
	if (file_exists(ABSPATH . 'wp-config.php'))
		echo 'wp-config.php currently permissions: ' . decoct(fileperms(ABSPATH . 'wp-config.php') & 0777);
	echo '<br>';
	echo '<br>';
	echo '<br>';
	...
	echo '<br>';
	if ($ctdf > 0)
		echo esc_attr($ctdf) . ' ' . __('Files with wrong permissions. This plugin will show max 50.', 'wptools');
	else
		...
		<div id="wptools-steps3">
			...
			if ($bill_install_memory['msg_type'] == 'ok') {
				if ($bill_install_memory['wp_limit'] - $bill_install_memory["usage"] < 30) {
					echo 'Your site free memory is too low and can happens memory fault error, also without install our plugin.';
					echo '<br>';
					echo "The WordPress Memory Limit is " . esc_attr($bill_install_memory['wp_limit']) . " MB";
					echo '<br>';
					echo "The WordPress Memory Usage is " . esc_attr($bill_install_memory['usage']) . " MB";
					echo '<br>';
					echo 'The free memory now is ' . esc_attr(($bill_install_memory['wp_limit']) - esc_attr($bill_install_memory["usage"])) . ' MB';
					echo '<br>';


					echo 'For more details, visit our site';
					echo '<a href="https://wpmemory.com/">WP MEMORY (Free WordPress Plugin to Fix Low Memory Limit)</a>';
				...
				echo "Unable to get ini_get('memory_limit')";
				echo '<br>';
				$bill_found_error = true;
			} elseif ($bill_install_memory['msg_type'] == 'notok(2)') {
				echo 'Unable to get memory_get_usage() ';
				echo '<br>';
				$bill_found_error = true;
			} elseif ($bill_install_memory['msg_type'] == 'notok(3)') {
				echo 'Memory Usage is not Numeric (3)';
				echo '<br>';
				$bill_found_error = true;
			} elseif ($bill_install_memory['msg_type'] == 'notok(4)') {
				echo 'Memory Usage < than 1';
				echo '<br>';
				$bill_found_error = true;
			} elseif ($bill_install_memory['msg_type'] == 'notok(5)') {
				echo 'Possible error: On file wp-config.php memory symbol should be M.';
				echo '<br>';
				echo 'For example,  define("WP_MEMORY_LIMIT", "256M"); ';
				echo '<br>';
				echo 'For more details, visit our site ';
				echo '<a href="https://wpmemory.com/">WP MEMORY (Free WordPress Plugin to Fix Low Memory Limit)</a>';
				...
				echo 'Memory Limit out of range. (6)';
				echo '<br>';
				$bill_found_error = true;
			} elseif ($bill_install_memory['msg_type'] == 'notok(7)') {
				echo 'General Failure (7) to get memory information.';
				echo '<br>';
				$bill_found_error = true;
			}
			if ($bill_found_error == false) {
				echo 'No Memory Problems found.';
				echo '<br>';
				echo 'The free memory now is ' . esc_attr($bill_install_memory['wp_limit'] - $bill_install_memory["usage"]) . ' MB.';
				echo '<br>';
				echo 'For more details about how manage and control the memory, visit our site ';
				echo '<a href="https://wpmemory.com/">WP MEMORY (Free WordPress Plugin to Fix Low Memory Limit)</a>';
				echo '<br>';
			}
			//$bill_found_error = false;
			echo '<h3>';
			echo "Step 2 - Check for Javascript Errors";
			echo '</h3>';
			echo '<div id="bill_javascript_status">';
			echo 'Javascript is NOT working in your site and/or with your browser!';
			echo '</div>';
			/* WP Tools ========= BEGIN ================  */
			//$bill_found_error = false;
			echo '<h3>';
			echo "Step 3 - Check for Hosting Resources";
			echo '</h3>';
			if (!defined('PHP_OS_FAMILY')) {
			?>
				Maybe Your server is not Linux
				<br>
				and this plugin requires Linux.
				<br>
				...
				?>
					Your server is not Linux
					<br>
					and this plugin requires Linux.
					<br>
					Your server is: &nbsp; <?php echo esc_attr(PHP_OS_FAMILY); ?>
					<br>
					<br>
				<?php
					$bill_found_error = true;
			...
			if (!function_exists('sys_getloadavg')) {
				?>
				Plugin requirement: &nbsp; PHP function sys_getloadavg enabled. Talk with your hosting and request to enable
				it.
				<br>
				<br>
				<?php
			...
			try {
				...
				?>
					Plugin requirement: &nbsp; /proc/cpuinfo doesn't readable. Talk with your hosting and request to enable it.
					<br>
					<br>
				<?php
					$bill_found_error = true;
			...
			try {
				if (!@is_readable('/proc/stat')) {
					...
					$bill_found_error_hosting = true;
					?>
					Plugin requirement: &nbsp; /proc/stat doesn't readable. Talk with your hosting and request to enable it.
					<br>
					<br />
				<?php
					$bill_found_error = true;
			...
			try {
				...
				?>
					Plugin requirement: &nbsp; /etc/os-release doesn't readable. Talk with your hosting and request to enable
					it.
					<br>
					<br />
				<?php
			...
			if (version_compare(PHP_VERSION, '7.0.0', '<')) {
				echo 'PHP Version 7.0 or bigger is required. My version: ' . PHP_VERSION . "\n";
				$bill_found_error = true;
				$bill_found_error_hosting = true;
				echo '<br>';
				echo '<br>';
			...
			<?php
				...
				//echo '<br>';
				echo 'If we are wrong, please, contact our support at: https://wptoolsplugin.com';
				echo '<br>';
			}
			...
			$site = '#';
			$bd_msg = '<br /><br />';
			$bd_msg .= '<a href="' . esc_attr($site) . '" class="button button-primary">Dismiss</a>';
			$bd_msg .= '<br /><br />';
			// echo $bd_msg;
			echo '</div>';
			echo '</big>';
...
function wptools_options_robots()
{
	...
	if (isset($_POST['wptools-robots-button'])) {
		if (isset($_POST['wtools-robots-textarea'])) {
			...
			if (!$handle_bkp = fopen($file_bkp, 'w+')) {
				echo "Cannot create BKP file - " . esc_attr($file_bkp);
				return;
			}
			if (fwrite($handle_bkp, $txt) === FALSE) {
				echo "Cannot write to Backup file - " . esc_attr($file_bkp);
				return;
			}
			if (!$handle = fopen($file, 'w+')) {
				echo "Cannot open file to save - " . esc_attr($file_bkp);
				return;
			}
			if (fwrite($handle, $txt) === FALSE) {
				echo "Cannot write to file - " . esc_attr($file_bkp);
				return;
			}
			echo '<b>';
			echo "Success, file robots.txt updated!";
			echo '<br>';
			echo 'Backup file created: ' . esc_attr($file_bkp);
			echo '</b>';
			echo '<br>';
			echo 'If you save again, will overwrite the backup.';
			echo '<br>';
			echo '<br>';
			fclose($handle);
		} else {
			echo 'Fail to Get Post Content!';
			return;
		}
	}
	if (!file_exists($file)) {
		echo 'File not found.';
		echo '<br>';
		echo 'To create, fill out the form below and click SAVE';
	} else {
		if (is_writable($file)) {
			$txt = trim(file_get_contents($file, true));
			$size = filesize($file);
		...
		if(!is_writable($file)){
				// echo substr(sprintf('%o', fileperms($file)), -4);
				echo 'File not writable. Check file permissions and user own. Unable to edit it.';
				echo '<br>';
				return;
			}
		}
		if (empty($txt)) {
			echo 'Empty file robots.txt! <br>';
			echo 'To create, fill out the form below and click SAVE';
		} else {
			echo 'Edit the file and click SAVE';
		}
	}
	...
	function wptools_options_php_info()
	{
		...
		/*
				...
				*/
		?>
		<h1><?php echo esc_attr('PHP Information', 'wptools'); ?></h1>
		<hr />
		<center>
			<h2><?php echo esc_attr('Basic Information', 'wptools'); ?></h2>
			<table class="wptools_admin_table">
				...
				<tbody>
					...
					<tr>
						<td><?php echo esc_attr('Server Timezone', 'wptools'); ?></td>
						<td>
							<?php
					...
					<tr>
						<td><?php echo esc_attr(__('PHP Max Memory Limit', 'wptools')); ?></td>
						<td>
					...
					<tr>
						<td><?php echo esc_attr('PHP Max Execution Time', 'wptools'); ?></td>
						<td><?php echo esc_attr(wptools_php_max_execution_time()) . " " . __("sec", "wptools");
							?></td>
						<td><?php
					...
					<tr>
						<td><?php echo esc_attr(__('PHP Max Input Vars', 'wptools')) . '*'; ?></td>
						<td><?php echo esc_attr(wptools_php_max_input_vars()); ?></td>
					</tr>
				</tbody>
			</table>
			<br />
			<a href="https://wptoolsplugin.com/blog/">(*) Check how to do it at our blog.</a>
			<h2><?php echo esc_attr('Complete Information', 'wptools'); ?></h2>
		</center>
	<?php
		ob_start();
	...
	function wptools_add_menu_items9()
	{
		global $wptools_checkversion;
		if (empty($wptools_checkversion)) {
			add_submenu_page(
